...
const companyLinks = [
  ...
  { label: "FAQ", path: "/faq" },
  { label: "Store Locations", path: "/store" },
  { label: "Franchise", path: "/franchise" },
];

const shopLinks = [
  { label: "Burgers", path: "/menu" },
  { label: "Fries & Sides", path: "/menu" },
  { label: "Combos & Deals", path: "/menu" },
  { label: "Beverages", path: "/menu" },
...
const Footer = () => {
  ...
  return (
    <footer className="w-full bg-cream pb-6 px-4 md:px-8 font-primary">
      {/* Taped card */}
      <div className="relative bg-cream rounded-3xl max-w-6xl mx-auto px-6 pb-6">
        {/* Tape top-left */}
        <div className="hidden md:block absolute -top-5 -left-6 scale-75 origin-top-left">
          {tape}
        </div>
        {/* Tape top-right */}
        <div className="hidden md:block absolute -top-5 -right-6 rotate-90 scale-75 origin-top-right">
          {tape}
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-10">
          ...
          <div className="flex flex-col gap-4 md:max-w-[220px] shrink-0">
            <Link to="/">
              <img src={isDark ? logoCream : logoBrown} alt="CrushBurg" className="w-28 h-auto object-contain" />
            </Link>
            <p className="text-sm text-muted leading-relaxed">
              Your go-to burger brand serving mouth-watering burgers, fries,
              and more — 100% vegetarian.
            </p>
            <div className="flex flex-col gap-1 text-sm">
              <a
                href={`tel:${CONTACT_NUMBER}`}
                className="flex items-center gap-2 text-muted hover:text-brand transition"
              >
                <Phone size={14} />
                {CONTACT_NUMBER}
              </a>
            ...
            <div className="flex gap-3 pt-1">
              <a href="#" aria-label="Facebook" className="w-9 h-9 flex items-center justify-center border border-cream-dark rounded-full hover:bg-brand hover:text-white hover:border-brand transition">
                <Facebook size={16} />
              </a>
              <a href="#" aria-label="Instagram" className="w-9 h-9 flex items-center justify-center border border-cream-dark rounded-full hover:bg-brand hover:text-white hover:border-brand transition">
                <Instagram size={16} />
              </a>
              <a href="#" aria-label="YouTube" className="w-9 h-9 flex items-center justify-center border border-cream-dark rounded-full hover:bg-brand hover:text-white hover:border-brand transition">
                <Youtube size={16} />
              </a>
              <a href="#" aria-label="Twitter" className="w-9 h-9 flex items-center justify-center border border-cream-dark rounded-full hover:bg-brand hover:text-white hover:border-brand transition">
                <Twitter size={16} />
              </a>
            </div>
          </div>

          {/* Right — desktop nav columns */}
          <div className="hidden md:flex flex-1 justify-end gap-12 lg:gap-20">
            {/* Our Company */}
            <div className="flex flex-col gap-4">
              <h4 className="uppercase text-xs font-semibold tracking-widest text-muted-light">
                Our Company
              </h4>
              <ul className="flex flex-col gap-2">
                {companyLinks.map((link) => (
                  <li key={link.path + link.label}>
                    <Link
                      to={link.path}
                      className="text-sm text-muted hover:text-brand transition whitespace-nowrap"
                    >
                      {link.label}
                    </Link>
                  </li>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-3">
                <p className="text-sm text-muted max-w-[180px] leading-relaxed">
                  Proven model. Strong ROI. Full support from day one.
                </p>
                <Link
                  to="/franchise"
                  className="inline-flex items-center gap-2 bg-brand text-white text-sm px-4 py-2 rounded-full font-medium hover:bg-brand-light transition w-fit"
                >
                ...
                <div className="flex gap-2">
                  <a
                    href={`mailto:${CONTACT_EMAIL}`}
                    className="text-xs text-white px-3 py-1.5 bg-brand rounded-full hover:bg-panel transition"
                  >
                    Email Us
                  </a>
                  <a
                    href={`tel:${CONTACT_NUMBER}`}
                    className="text-xs text-white px-3 py-1.5 bg-green-500 rounded-full hover:bg-green-600 transition"
                  >
                    Call Us
                  </a>
                </div>
          ...
          {/* Mobile accordion */}
          <div className="md:hidden flex flex-col divide-y divide-cream-dark border-t border-cream-dark">
            {[
              { key: "company", label: "Our Company", items: companyLinks, type: "links" },
              { key: "shop", label: "Shop", items: shopLinks, type: "links" },
              { key: "franchise", label: "Franchise", type: "franchise" },
            ].map(({ key, label, items, type }) => (
              <div key={key} className="py-3">
                <button
                  onClick={() => toggleSection(key)}
                  className="w-full text-left flex justify-between items-center font-semibold text-brand text-sm"
                >
                  {label}
                  <span className="text-lg leading-none">{openSection === key ? "−" : "+"}</span>
                </button>
                {openSection === key && (
                  <div className="mt-3">
                    {type === "links" && (
                    ...
                    {type === "franchise" && (
                      <div className="flex flex-col gap-3">
                        ...
                        <Link
                          ...
                          className="inline-flex items-center gap-2 bg-brand text-white text-sm px-4 py-2 rounded-full font-medium hover:bg-brand-light transition w-fit"
                        >
                          <Users size={14} /> Explore Franchise
                        </Link>
                        <div className="flex gap-2">
                          <a
                            href={`mailto:${CONTACT_EMAIL}`}
      ...
      {/* Bottom bar */}
      <div className="max-w-6xl mx-auto mt-4 px-2 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 text-sm text-muted-light">
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-6 items-start sm:items-center">
          <p className="whitespace-nowrap">© {currentYear} CrushBurg. All rights reserved.</p>
          <div className="flex gap-4">
            {legalLinks.map((link) => (
              <Link key={link.path} to={link.path} className="hover:text-brand transition whitespace-nowrap">
                {link.label}
              </Link>
            ))}
          </div>
        ...
        <div className="flex gap-3 items-center">
          <a href="#" aria-label="Facebook" className="hover:text-brand transition"><Facebook size={16} /></a>
          <a href="#" aria-label="Instagram" className="hover:text-brand transition"><Instagram size={16} /></a>
          <a href="#" aria-label="YouTube" className="hover:text-brand transition"><Youtube size={16} /></a>
          <a href="#" aria-label="Twitter" className="hover:text-brand transition"><Twitter size={16} /></a>
        </div>
      </div>
    </footer>
  );
