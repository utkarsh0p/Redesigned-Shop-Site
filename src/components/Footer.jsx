...
const Footer = () => {
  ...
  return (
    <footer className="w-full bg-offwhite pb-6 px-4 md:px-8 font-primary">
        ...
        <div className="flex flex-col md:flex-row gap-10">
          ...
          <div className="flex flex-col gap-4 md:max-w-[220px] shrink-0">
            ...
            <div className="flex flex-col gap-1 text-sm">
              <a
                ...
                className="flex items-center gap-2 text-gray-500 hover:text-red-dark transition"
              >
                <Phone size={14} />
                {CONTACT_NUMBER}
              </a>
            ...
            <div className="flex gap-3 pt-1">
              <a href="#" aria-label="Facebook" className="w-9 h-9 flex items-center justify-center border border-gray-200 rounded-full hover:bg-red-dark hover:text-white hover:border-red-dark transition">
                <Facebook size={16} />
              </a>
              <a href="#" aria-label="Instagram" className="w-9 h-9 flex items-center justify-center border border-gray-200 rounded-full hover:bg-red-dark hover:text-white hover:border-red-dark transition">
                <Instagram size={16} />
              </a>
              <a href="#" aria-label="YouTube" className="w-9 h-9 flex items-center justify-center border border-gray-200 rounded-full hover:bg-red-dark hover:text-white hover:border-red-dark transition">
                <Youtube size={16} />
              </a>
              <a href="#" aria-label="Twitter" className="w-9 h-9 flex items-center justify-center border border-gray-200 rounded-full hover:bg-red-dark hover:text-white hover:border-red-dark transition">
                <Twitter size={16} />
              </a>
            </div>
          </div>
          ...
          <div className="hidden md:flex flex-1 justify-end gap-12 lg:gap-20">
            ...
            <div className="flex flex-col gap-4">
              <h4 className="uppercase text-xs font-semibold tracking-widest text-gray-400">
                Our Company
              </h4>
              <ul className="flex flex-col gap-2">
                {companyLinks.map((link) => (
                  <li key={link.path + link.label}>
                    <Link
                      to={link.path}
                      className="text-sm text-gray-500 hover:text-red-dark transition whitespace-nowrap"
                    >
                      {link.label}
                    </Link>
                  </li>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-3">
                ...
                <Link
                  to="/franchise"
                  className="inline-flex items-center gap-2 bg-red-dark text-white text-sm px-4 py-2 rounded-full font-medium hover:bg-red-light transition w-fit"
                >
                ...
                <div className="flex gap-2">
                  <a
                    href={`mailto:${CONTACT_EMAIL}`}
                    className="text-xs text-white px-3 py-1.5 bg-red-dark rounded-full hover:bg-black transition"
                  >
                    Email Us
                  </a>
                  <a
          ...
          {/* Mobile accordion */}
          <div className="md:hidden flex flex-col divide-y divide-offwhite-dark border-t border-offwhite-dark">
            {[
              ...
              <div key={key} className="py-3">
                <button
                  ...
                  className="w-full text-left flex justify-between items-center font-semibold text-red-dark text-sm"
                >
                  {label}
                      ...
                      <ul className="flex flex-col gap-2">
                        {items.map((link) => (
                          <li key={link.label}>
                            <Link
                              to={link.path}
                              className="text-sm text-gray-500 hover:text-red-dark transition"
                            >
                              {link.label}
                            </Link>
                          </li>
      ...
      {/* Bottom bar */}
      <div className="max-w-6xl mx-auto mt-4 px-2 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 text-sm text-gray-400">
        <div className="flex flex-col sm:flex-row gap-2 sm:gap-6 items-start sm:items-center">
          ...
          <div className="flex gap-4">
            {legalLinks.map((link) => (
              <Link key={link.path} to={link.path} className="hover:text-red-dark transition whitespace-nowrap">
                {link.label}
              </Link>
            ))}
          </div>
        ...
        <div className="flex gap-3 items-center">
          <a href="#" aria-label="Facebook" className="hover:text-red-dark transition"><Facebook size={16} /></a>
          <a href="#" aria-label="Instagram" className="hover:text-red-dark transition"><Instagram size={16} /></a>
          <a href="#" aria-label="YouTube" className="hover:text-red-dark transition"><Youtube size={16} /></a>
          <a href="#" aria-label="Twitter" className="hover:text-red-dark transition"><Twitter size={16} /></a>
        </div>
      </div>
    </footer>
  );
